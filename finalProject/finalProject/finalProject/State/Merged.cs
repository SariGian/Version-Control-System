...
{
    internal class Merged : SystemItemState
    {
        public Merged(SystemItem SystemItem) : base(SystemItem)
        {
        }
        ...
        //{
        //    if (SystemItem is Folder)
        //        ((Folder)SystemItem).Items = ((FolderMemento)SystemItem.Taker.Pop()).Content;
        //    else ((FinalProject.Composite.File)SystemItem).Content = ((FileMemento)SystemItem.Taker.Pop()).Content;
        //    return $"the item {SystemItem.Name} returned to its previous state.";
        //}
